...
Most of my LLM calls were embarrassingly simple — slap on a label, pull out a field, sort into a bucket. Am I paying sports-car prices for grocery runs?

So I ran the numbers. A sentiment pipeline pushing 500k calls/month through Claude Sonnet 4 (100-token in, 20-token out) rings up at roughly **$900/mo**. The same job on a fine-tuned 4B model renting a single A100? **$90/mo** — with a one-time training bill under $5.

Small model. Big savings. 10× big.

That was enough to send me down the rabbit hole. This post is what I found: when small language models actually earn their keep, when they don't, and a calculator so you can punch in your own numbers and see for yourself.
